...
const NewTweet = ({updateTweets}) => {
  ...
  return (
    <div className={classes.newTweet}>
      <Grid container>
        <img src={getImage()} style={{width: 60, height: 60, borderRadius: '50%'}} alt='img'/>
        <input placeholder={("What's happening?")} className={classnames(classes.input)}
      ...
      <Grid container direction={"row-reverse"} style={{marginTop: 16}}>
        <Button variant={"contained"} color={"primary"}
                className={classes.newTweetBtn} onClick={newTweetClick}>{("Tweet")}</Button>
